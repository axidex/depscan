# Running craftnovate in Sourcecraft CI
        ...
        cubes:
          ...
          - name: preview
            script:
              ...
              - $(go env GOPATH)/bin/craftnovate --repo .
          - name: open-prs
            env:
...
- Scans Gradle (`build.gradle.kts`, `gradle.properties`, `libs.versions.toml`),
  ...
  branch `craftnovate/<coord>-<target>`. **Idempotent** — a dependency whose
...
- Honors `craftnovate.json` (ignoreUnstable / allowedVersions / ignoreDeps /
  packageRules / groupName / labels / prConcurrentLimit) if present.

## Dry-run locally first

```bash
craftnovate --repo /path/to/project          # preview the plan
craftnovate --repo /path/to/project --write   # apply edits locally (no PR)
```
