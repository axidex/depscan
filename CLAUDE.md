...
- Docker: `make docker-build` / `make docker-run REPO=…`. Multi-stage `Dockerfile`
  (static binary → alpine + git/openssh/ca-certs + internal Yandex root CAs via
  `crls.yandex.net`, non-root, `safe.directory '*'`); the `image` job in
  `release.yml` publishes the multi-arch image to `ghcr.io/axidex/craftnovate`.
...
- `release.yml` has two jobs: `release` (GoReleaser binaries; outputs the resolved
  tag) and `image` (`needs: release`, `buildx`+QEMU multi-arch → GHCR from the
  `Dockerfile`, `packages: write`). The image lives in the release workflow on
  purpose: a tag pushed by `GITHUB_TOKEN` on `workflow_dispatch` does NOT trigger a
  separate on-push-tag workflow, so a standalone docker.yml skipped dispatch releases.
