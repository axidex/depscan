...
func open(ctx context.Context, git *Git, plat Platform, repoID, base, remote string, grp PRGroup) (*sourcecraft.PullRequest, error) {
	// Branch off the checked-out commit (HEAD) — always valid, including a
	// detached CI checkout — while the pull request targets the real `base`
	// branch. Using `base` as the start point would fail when it isn't a local
	// ref (e.g. "main" in a detached checkout).
	if err := git.PushBranchWithEdits(ctx, "HEAD", grp.Branch, grp.Title, remote, func(dir string) error {
